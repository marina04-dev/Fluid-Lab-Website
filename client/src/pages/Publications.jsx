...
const Publications = () => {
  ...
  // type filter state variables
  const [selectedType, setSelectedType] = useState("all");
  // search term filter state variables
  ...
  // check the applied filters whenever any of the filters changes & fetch the publications that fullfill the requirements
  useEffect(() => {
    const filters = {};
  ...
  const availableYears = [...new Set(publications.map((pub) => pub.year))].sort(
    (a, b) => b - a
  );
        ...
        <div className="container-custom">
          <div className="text-center text-white">
            ...
            <p className="text-xl opacity-90 max-w-3xl mx-auto">
              Explore our research contributions and academic publications in
              fluid mechanics
            </p>
          </div>
        </div>
                      ...
                      {publication.projects &&
                        publication.projects.length > 0 && (
                          <div className="text-sm text-gray-500">
                            Related to {publication.projects.length} project
                            {publication.projects.length !== 1 ? "s" : ""}
                          </div>
                        )}
                    </div>
                  </div>
